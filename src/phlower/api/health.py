...
async def healthz(request: Request) -> HealthResponse:
    ...
    return HealthResponse(
        ...
        sqlite_rows=sqlite_store._cached_row_count if sqlite_store else None,
        sqlite_detail_rows=sqlite_store._cached_detail_row_count if sqlite_store else None,
        sqlite_oldest_at=sqlite_store._cached_oldest_at if sqlite_store else None,
        disk_usage_pct=round(sqlite_store.disk_usage_pct(), 1) if sqlite_store else None,
        disk_free_mb=round(sqlite_store.disk_free_mb()) if sqlite_store else None,
